...
describe("rehomeDigipet", () => {
    test("when there is a digipet, it is 'rehomed', freeing up space to hatch another digipet", () => {
        // setup
        model_1.setDigipet({ happiness: 50, nutrition: 50, discipline: 50 });
        // act
        const digipet = controller_1.rehomeDigipet();
        // assert
        expect(digipet).toBe(undefined);
        expect(model_1.getDigipet()).toBe(null);
    });
    test("when there is no current digipet, it throws an error", () => {
        // setup
        model_1.setDigipet(undefined);
        // asert error gets thrown
...
describe("ignoreDigipet", () => {
    it("decrease all stats by 10 (to represent holistic need for attention)", () => {
        // setup
        model_1.setDigipet(model_1.INITIAL_DIGIPET);
        expect(model_1.getDigipet()).toStrictEqual(model_1.INITIAL_DIGIPET);
        // act
